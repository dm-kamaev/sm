...
var findActivityMatches = function(schools, activityNumber, activityName) {
    var ourSchools = {};
    for (var i = 0, schoolsLength = schools.length; i < schoolsLength; i++) {
        if (findMatch(schools[i], activityNumber, activityName)) {
...
var fillActsDB = function(schools, activities) {
    ...
    for (var i = 0, actsLength = activities.length; i < actsLength; i++) {
        ...
        if (activities[i].groupLength <= 255) {
            ourSetSchools = findActivityMatches(schools, activities[i].number, activities[i].name);
        }
